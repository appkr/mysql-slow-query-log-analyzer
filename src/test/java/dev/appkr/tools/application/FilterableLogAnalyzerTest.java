...
class FilterableLogAnalyzerTest {
  ...
  static final String REPORT_ID = "test";
  FilterableLogAnalyzer sut;

  @Test
  void noFilter() throws IOException {
    final AnalysisReport filteredReport = sut.analyze(REPORT_ID, null, null, null, null);
    assertThat(filteredReport.getLogEntries().size()).isEqualTo(2);
  }

  @Test
  void filter1KMillis_thenExpectOneEntry() throws IOException {
    final AnalysisReport filteredReport = sut.analyze(REPORT_ID, null, 1000, null, null);
  ...
  void sortByQueryTime_thenExpectFirstItemQueryIdIs2() throws IOException {
    final AnalysisReport filteredReport = sut.analyze(REPORT_ID, null, null, null, Sort.by("queryTime"));
    assertThat(filteredReport.getLogEntries().get(0).getQueryId()).isEqualTo("2");
  }
...
SELECT 1 FROM DUAL;
      ...
      return new AnalysisReport(REPORT_ID, List.of(new SlowQueryLog(logString1), new SlowQueryLog(logString2)));
    }
  }
}
